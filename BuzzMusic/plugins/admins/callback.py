...
from BuzzMusic import YouTube, app
from BuzzMusic.core.call import Buzz
from BuzzMusic.misc import SUDOERS, db
from BuzzMusic.utils.database import (is_active_chat,
                                       is_music_playing, is_muted,
                                       is_nonadmin_chat, music_off,
                                       music_on, mute_off, mute_on,
                                       set_loop)
from BuzzMusic.utils.decorators.language import languageCB
from BuzzMusic.utils.formatters import seconds_to_min
from BuzzMusic.utils.inline.play import (panel_markup_1,
                                          panel_markup_2,
                                          panel_markup_3,
                                          stream_markup,
                                          telegram_markup)
from BuzzMusic.utils.stream.autoclear import auto_clean
from BuzzMusic.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Buzz.pause_stream(chat_id)
        ...
        await Buzz.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
            _["admin_4"].format(mention)
        )
    elif command == "Stop" or command == "End":
        await CallbackQuery.answer()
        await Buzz.stop_stream(chat_id)
        ...
        await Buzz.mute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await Buzz.unmute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Buzz.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await Buzz.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await Buzz.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
